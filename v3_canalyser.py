...
#       determine dependent vs independent variables
    ...
    while not all_guesses_done:
        ...
        predicted_readings["gear"] = [sign(r) for r in components[guesses_comp["avg_vel"]].readings_quantised]
...
#        if len(relations[rel][1]) == 3:
#            print(f"{rel}: throttle: {relations[rel]}")
#            pass #it is throttle
#        elif len(relations[rel][1]) == 1 and len(relations[rel][-1]) == 1:
#            print(f"{rel}: steering: {relations[rel]}")
#            pass #it is steering
#        for opp_id in opp_comp_ids:
#
#            for c2 in comp_two:
#                print(comp_two[c2].id)
#                for r in comp_two[c2].readings:
#                    val = int(r[1],16)
#                    if val > 127 and components[c2].is_twos_complement: val -=256
#                print(comp_two[c2].readings)
# now send max val, listen for other id
        
        

#test sequence:
# play min value, mid value, max value
# look at top abs(spearman) comps
# case throttle:
#   will go up: wheels, vel
#   will go down: none
# case steering:
#   will go up: right wheel
#   will go down: left wheel
#   will stay same: vel
# case gear:
#   no effect
# case output:
...
# test each pair of highly spearman comps to determine which is the independent and which is the dependent?



#sends a random value from unique values for 0.5s
#cannot be the same as previous

#then what

#case throttle:
#	abs wheels, vel increase
#		therefore need avg abs vals for these

#case gear:
#	spearman throttle -> wheels, vel decreases

#case steering:#
#	one wheel increases, other decreases
#	vel increases

#process:
#send min val x secs, mid val x secs, max val x secs
